...
class Config:
    ...
    source_table: str = "GLOBALLEGENDS_CLEANED"
    target_table: str = "TA_STANDARD_GLOBALLEGENDS"
    read_chunk: int = 200_000
    parse_workers: int = 4
    max_legs: int = 5


PREFIX_FLIGHTNO="FLIGHT_NUMBER"
PREFIX_DEPARTUREDATE="FLIGHT_DATE"
...
class ReferenceData:
    ...
    def __init__(self, con: duckdb.DuckDBPyConnection):
        ...
        self.srb_airports: frozenset = SRB_AIRPORTS
...
class ChunkProcessor:
    ...
    def __init__(
        ...
        self.srb_airports = srb_airports
    ...
    def process(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # NOTE: IsMultiSegment intentionally left as a placeholder (None) here,
        # same as upstream — not computed in this pass.
        placeholders = {
    ...
    def _extract_leg(self, df: pd.DataFrame, leg_num: int) -> Optional[pd.DataFrame]:
        ...
        # Per-leg airline column (e.g. "AirlineCode1", "AirlineCode2"...), falling
        # back to a flight-number-derived airline when the per-leg column is
        # missing or blank for a given row.
        al_col = f"{PREFIX_AIRLINE}"

        # Check if columns exist (you have up to 6 flights)
        # Airport columns must exist too -- ap_to reaches one past max_legs
        # (e.g. "Airport6" for leg 5) and may not be present in the source.
        if (
            fn_col not in df.columns
            or fd_col not in df.columns
            or ap_from not in df.columns
            or ap_to not in df.columns
        ):
            return None
        ...
        if al_col in sub.columns:
            ...
            # Fallback: if AirlineCode{n} is blank/NaN for a row, derive it from the
        ...
        return pd.DataFrame(
            {
                ...
                "PaxName": sub["PAX_NAME"].fillna("").astype(str).str.strip().values,
                "ETicketNo": sub["TK_NUMBER"].fillna("").astype(str).str.strip().values,
                "BookingRef": None,
    ...
    def _vectorized_eligibility(self, df: pd.DataFrame) -> pd.Series:
        ...
        Rule 3b (Serbian carve-out)
            If the journey starts in Serbia (SRB_AIRPORTS) and the first
            operating carrier is a SRB_CARRIER (JU), the journey IS eligible.
        ...
        Higher-priority rules overwrite the results of lower-priority rules.
            ...
            Rule5 -> Rule4 -> Rule3 -> Rule3b -> Rule2 -> Rule1
        ...
        first_is_srb = first_leg_airport.isin(self.srb_airports)

        airline1_is_special = airline1.isin(SPECIAL_CARRIERS)              # LH/XQ/QR on leg1
        airline1_is_srb_carrier = airline1.isin(SRB_CARRIERS)              # JU on leg1
        ...
        # RULE 3b: journey starts in Serbia on a Serbian carrier -> Eligible True
        rule3b_true = first_is_srb & airline1_is_srb_carrier
        ...
        eligible = eligible.mask(rule3b_true, True)    # Rule 3b (Serbian carve-out)
