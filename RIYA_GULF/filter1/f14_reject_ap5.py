...
SOURCE_TABLE = "RIYA_GULF_SPLIT3"
TARGET_TABLE = "RIYA_GULF_SPLIT4"
...
print(f"Rows to reject (AP5 IS NOT NULL)    : {reject_count:,}")
...
con.execute(f"""
    ...
    SELECT
        *,
        'AP5 IS NOT NULL' AS REJECTIONREASON
    FROM {SOURCE_TABLE}
    WHERE {reject_condition}
""")
...
rejected_inserted_count = con.execute(f"""
    SELECT COUNT(*)
    FROM {REJECT_TABLE}
    WHERE REJECTIONREASON = 'AP5 IS NOT NULL'
""").fetchone()[0]
...
print(f"Rejected (AP5 IS NOT NULL)   : {reject_count:,}")
