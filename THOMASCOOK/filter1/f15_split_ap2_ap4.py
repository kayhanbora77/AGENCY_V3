...
WHERE ((AIRPORT2 = AIRPORT4) AND AIRPORT6 IS NOT NULL AND AIRPORT7 IS NULL)
"""
...
def find_all_split_points(flights, airports):
    """
    Rule 1 — Exactly 5 flights, Airport2 == Airport4:
        e.g. BOM → LHR → BHD → LHR → BOM → DEL
        Split into:
          Segment 1: FlightNumber1,FlightNumber2 | Airport1 → Airport2 → Airport3
          Segment 2: FlightNumber3,FlightNumber4,FlightNumber5 | Airport3 → Airport4 → Airport5 → Airport6
    """
    n_f = len(flights)
    n_a = len(airports)
    ...
    # ── Rule 1: exactly 5 flights and Airport2 == Airport4 ───────────────────
    if n_f == 5 and n_a == 6 and airports[1] == airports[3]:
        split_points.add(2)
...
def process_table(db_path=DB_PATH, table=SOURCE_TABLE, batch_size=BATCH_SIZE):
    ...
    filtered_total = con.execute(f"""
        ...
        {AP2_AP4_WHERE_CLAUSE}
    """).fetchone()[0]
    print(f"\n  Step 2: Processing {filtered_total:,} 5-flight rows for split...")
