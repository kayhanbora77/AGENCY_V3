...
WHERE ((AIRPORT2 = AIRPORT5) AND AIRPORT6 IS NOT NULL AND AIRPORT7 IS NULL)
"""
...
def find_all_split_points(flights, airports):
    """
    Rule 1 — Exactly 5 flights, Airport2 == Airport5:
        e.g. DEL → LHR → LAS → JFK → LHR → DEL
        Split into:
    ...
    # ── Rule 1: exactly 5 flights and Airport2 == Airport5 ───────────────────
    if n_f == 5 and n_a == 6 and airports[1] == airports[4]:
        split_points.add(3)
...
def process_table(db_path=DB_PATH, table=SOURCE_TABLE, batch_size=BATCH_SIZE):
    ...
    # ── Step 2: Count only the 5-flight rows to process ──────────────────────
