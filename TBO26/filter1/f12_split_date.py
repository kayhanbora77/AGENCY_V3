...
ParentId logic
--------------
- ParentId is set ONLY when a row is actually split in THIS step
  (children get ParentId = original Id).
- Unsplit rows keep whatever ParentId they already have (do NOT force NULL).
"""
...
SOURCE_TABLE = "TBO26_SPLIT"
TARGET_TABLE = "TBO26_SPLIT2"
...
def build_child_row(parent_list, flights_slice, airports_slice, parent_id):
    ...
    ParentId is set because this row is a product of a split.
    """
    ...
    child[COL_IDX["ParentId"]] = str(parent_id)   # only set for split children
...
def process_batch(rows_df, all_cols):
    ...
    for row_list in records:
        ...
        if not split_points:
            # NOT split → keep existing ParentId as-is (do NOT force NULL)
            unsplit_rows.append(trimmed_row)
            continue

        # Split the row → children get ParentId = original Id
...
def process_table(db_path=DB_PATH, table=SOURCE_TABLE, batch_size=BATCH_SIZE):
    ...
    while True:
        ...
        if not children_df.empty:
            ...
            # Count distinct parents that produced children
