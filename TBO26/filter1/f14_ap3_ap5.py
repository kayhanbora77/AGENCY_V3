...
SOURCE_TABLE = "TBO26_SPLIT3"
TARGET_TABLE = "TBO26_SPLIT4"
...
WHERE (Airport3=Airport5) AND Airport7 IS NOT NULL
"""
...
def find_all_split_points(flights, airports):
    ...
    Rule 1 — Exactly 6 flights, Airport3=Airport5:
        e.g. AMD → BOM → CDG → IAH → CDG → BOM → AMD
        Split into:
          Segment 1: FlightNumber1,FlightNumber2,FlightNumber3 | Airport1 → Airport2 → Airport3 → Airport4
          Segment 2: FlightNumber4,FlightNumber5,FlightNumber6 | Airport4 → Airport5 → Airport6 → Airport7
    """
    n_f = len(flights)
    n_a = len(airports)
...
def process_table(db_path=DB_PATH, table=SOURCE_TABLE, batch_size=BATCH_SIZE):
    ...
    # ── Step 2: Count only the 6-flight rows to process ──────────────────────
    filtered_total = con.execute(f"""
        SELECT COUNT(*)
        FROM "{table}"
        {AP3_AP5_WHERE_CLAUSE}
    """).fetchone()[0]
    print(f"\n  Step 2: Processing {filtered_total:,} 6-flight rows for split...")
