...
WHERE (Airport1=Airport3) AND (Airport5 IS NULL)
"""
...
# ============================================================================
# SHARED BLANK-VALUE HELPER
# ============================================================================

# String placeholders that sometimes leak into VARCHAR columns from earlier
# pandas-based ETL stages that didn't properly convert missing values to
# SQL NULL before writing (e.g. str(np.nan) -> "nan", str(None) -> "None").
_BLANK_STRINGS = {"", "nan", "none", "null", "nat", "<na>"}


def _is_blank(val) -> bool:
    """
    True if val is None, NaN/NaT, or a string that is empty/whitespace-only
    or one of the known "looks blank but isn't a real NULL" placeholders.
    This is the shared blank-check pattern used across the pipelines to
    avoid the recurring "pandas NaN vs None" miscount bug.
    """
    if val is None:
        return True
    try:
        ...
        pass
    if isinstance(val, str) and val.strip().lower() in _BLANK_STRINGS:
        return True
    return False
...
def find_all_split_points(flights, airports):
    ...
    n_a = len(airports)
    print(
        f"FlightNumber= {n_f} {[fn for fn, _ in flights]}, "
    ...
    if n_f == 2 and airports[0] == airports[2]:
        split_points.add(1)
...
def process_table(db_path=DB_PATH, table=SOURCE_TABLE, batch_size=BATCH_SIZE):
    ...
    while True:
        ...
        # Unsplit rows (Airport3 != Airport5) already exist in TARGET — skip
