...
SOURCE_TABLE = "GILPIN_RAW"
TARGET_TABLE = "GILPIN_SPLIT"
REJECT_TABLE = "GILPIN_REJECT"
...
# ============================================================================
# COLUMN LISTS  —  match GILPIN_RAW schema exactly
# ============================================================================

FLIGHT_COLS = [f"{FLIGHT_PREFIX}{i + 1}" for i in range(MAX_FLIGHTS)]
DATE_COLS = [f"{DATE_PREFIX}{i + 1}" for i in range(MAX_DATES)] 
AIRPORT_COLS = [f"Airport{i + 1}" for i in range(MAX_AIRPORTS)] 
DYNAMIC_COLS = FLIGHT_COLS + DATE_COLS + AIRPORT_COLS

_RE_FLTNO = re.compile(r"^[A-Z]{2,3}\d+$")
_RE_SCI_NOTATION = re.compile(r"^(\d+)(?:\.0+)?E\+?(\d+)$", re.IGNORECASE)
_RE_FLTNO_WITH_SPACE = re.compile(r"^([A-Z]{1,3})\s+(\d+)$", re.IGNORECASE)
